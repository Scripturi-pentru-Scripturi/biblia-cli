BEGIN {
	...
	MAX_WIDTH = 80
	if (ENVIRON["BIBLIA_MAX_WIDTH"] ~ /^[0-9]+$/) {
		if (int(ENVIRON["BIBLIA_MAX_WIDTH"]) < MAX_WIDTH) {
			MAX_WIDTH = int(ENVIRON["BIBLIA_MAX_WIDTH"])
		}
	}
...
function printverse(verse,    word_count, characters_printed) {
	if (ENVIRON["BIBLIA_NOLINEWRAP"] != "" && ENVIRON["BIBLIA_NOLINEWRAP"] != "0") {
		printf("%s\n", verse)
		return
	}
